...
|:-----------|:------------|:------------|:------------|
| el | タグ・タグ名 | `string`  `Element`  `HTMLElement` | 指定したタグここで指定したタグの中身のpathを取得
| paths | パスの配列 | `NodeListOf<SVGGeometryElement>` | pathの配列、上よりここで指定したpathが優先
...
https://github.develo.org/scroll-parallax-effect/dist/scroll-parallax-effect/scroll-parallax-effect-svg.min.js

### Vanilla
```javascript
<script type="text/javascript" src="../js/scroll-parallax-effect-svg.min.js"></script>
```

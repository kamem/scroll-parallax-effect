  ...
  install(Vue, opt) {
    ...
    Vue.directive('parallax-timing', {
      [beforeMount]: (el, { value }, d) => {
        ...
        setTimeout(() => {
          const timing = new p.ParallaxTiming(el, opt, {
    ...
    Vue.directive('parallax-fit', {
      [beforeMount]: (el, { value }, d) => {
        ...
        setTimeout(() => {
          const element = opt.el || el;
          const fit = new p.ParallaxFit(element, opt, {
            targetPercentage: opt?.targetPercentag,
            threshold: opt?.threshold,
            status: opt?.status,
          });
    ...
    Vue.mixin({
      methods: {
        ...
        parallaxTiming(opt = {}) {
          return new p.ParallaxTiming(opt.target || opt.el, opt, {
            targetPercentage: opt?.targetPercentag,
            threshold: opt?.threshold,
            status: opt?.status,
          });
        ...
        parallaxFit(opt) {
          return new p.ParallaxFit(opt.el, opt, {
            targetPercentage: opt?.targetPercentag,
            threshold: opt?.threshold,
            status: opt?.status,
          });

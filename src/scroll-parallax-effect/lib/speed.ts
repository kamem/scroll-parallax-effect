...
export default class Speed {
  ...
  generateStyles(styles: CSSStyleDeclarationName[]) {
    return styles.map((name, i: number) => {
      const contentScrollPositionStyleValues =
        this.contentScrollPositionStyleValues[i] ||
        (this.el
          ? (document.defaultView?.getComputedStyle(this.el, null)[
              generateCamelCaseStyle(name)
            ] as string | number)
          : 0);
  ...
  generateValues(status: ScrollStatus, style: Style) {
    return style.styleValues.map((value, j) => {
      let _speed =
        typeof style.speed === 'object' ? style.speed[j] : style.speed;
      _speed = typeof _speed === 'number' ? _speed : 2;
      let newValue =
        -(
          -status.scrollPosition * _speed +
          scrollPositionStringToNumber(this.contentScrollPosition, status) *
            _speed
        ) + value;

      const _min =
        typeof style.min === 'object' ? style.min && style.min[j] : style.min;
      const _max =
        typeof style.max === 'object' ? style.max && style.max[j] : style.max;
    ...
    return this.styles.reduce((result, style) => {
      return Object.assign({}, result, {
        [style.name]: generateStyleValueString(
          ...
          this.generateValues(status, style)
        ),
      });
    }, {});
  }

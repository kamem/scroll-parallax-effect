...
export default class Timing {
  ...
  getEventScrollElementPosition(status: ScrollStatus) {
    return scrollPositionStringToNumber(
  ...
  timingEvent(status: ScrollStatus) {
    const isLineOver =
      status.scrollPosition >= this.getEventScrollElementPosition(status);
    ...
    if (isLineOver !== this.isLineOver) {
      ...
      const element =
        typeof this.el === 'string'
          ? document.querySelector<HTMLElement>(this.el)
          : this.el;

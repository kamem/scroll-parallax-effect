...
export  const setScrollEvents = (
  ...
  const isNewScrollPosition = !!(targetPercentage && (targetPercentage !== status.targetPercentage)) || !!(threshold && (threshold !== status.threshold))
...
export const getStringColor = (styleValue: string) => {
  const colors: {[key: string]: string} = { red: 'f00', blue: '00f', yellow: 'ff0', green: '008000' }
